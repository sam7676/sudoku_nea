# Constraint class
# Will return the next hint, or number of hints to complete, or number of bowman guesses, or answer
# Optimise with numpy -> 9x9x9 ?

# Algorithm X class
# Returns the number of solutions

# Generator class
# Returns an unfinished grid, which is dependent on the level we want to generate at
...
# We assume correctness, with algorithm X verifying only 1 solution exists
class constraint:
...
if testing == True:
    ...
    x = algorithm_x('800000000003600000070090200050007000000045700000100030001000068008500010090000400')
    assert x.sols == 1

    x = algorithm_x('820000000003600000070090200050007000000045700000100030001000068008500010090000400')
    assert x.sols == 0

    x = algorithm_x('000000000003600000070090200050007000000045700000100030001000068008500010090000400')
    assert x.sols > 1
    ...
    z = constraint('800000000003600000070090200050007000000045700000100030001000068008500010090000400')
    assert z.ans == '812753649943682175675491283154237896369845721287169534521974368438526917796318452'